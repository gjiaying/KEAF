...
class FAEA(framework.frameworks.MLFSModel):
    ...
    def __init__(self, encoder, max_length, weight_factor, device_ids, att):
        ...
        self.a = torch.from_numpy(np.diag(np.ones(max_length - 1, dtype=np.int32),1)).cuda()
        #对角线上移一位
        self.b = torch.from_numpy(np.diag(np.ones(max_length, dtype=np.int32),0)).cuda()
        #对角线
        self.c = torch.from_numpy(np.diag(np.ones(max_length - 1, dtype=np.int32),-1)).cuda()
        #对角线下移一位
        self.tri_matrix = torch.from_numpy(np.triu(np.ones([max_length,max_length], dtype=np.float32),0)).cuda()
    ...
    def forward(self, support, query, N, K, Q, anchor, support_label):
        ...
        rel_text_glo, rel_text_loc = self.encoders(anchor, 'SimpleFSRE')  # (B * N, D), (B * N, L, D)
       ...
       #[4,10,10]
        ...
        if not is_eval:
            ...
            sim_scalar = sim_scalar.repeat(Q, 1).t().reshape(-1)  # (B*totalQ)
            #[40]
        return logits, pred, logits_proto, labels_proto, sim_scalar
    ...
    def phase_attn(self,input_score1, mask1, input_score2, mask2, input_loc):    
        ...
        #2)连续性
        cons_phase_score=torch.zeros(constituent.size(0),self.max_length).cuda()
        #[10,128] 
        for i in range(constituent.size(0)):
            ...
            elif seg_i[i]==self.max_length-1:
                cons_phase_score[i][seg_i[i]]+=d[0][seg_i[i]-1]               
            else:
                cons_phase_score[i][seg_i[i]]+=torch.max(d[0][seg_i[i]-1]+d[0][seg_i[i]+1])

        #3)related
        related_phase_score =torch.zeros(constituent.size(0),self.max_length).cuda()
        #[10,5,128]
        ...
        related_phase_score = self.l2norm(related_phase_score)#(related_phase_score)
    ...
    def set_init_edge(self,input_loc,B,N,K):
        ...
        score=F.normalize(score)#[1,10,10]
        start=1
        full_label=[]
        for i in range(N):
            for j in range(K):
